...
/**
 * Disclaimer: This is a working test class for the tripSearcher functions on
 * the database. It expects that there are 2 separate databases built from the
 * carpools schema for containing data which is to remain unchanged:
 * dummyDatabase and searchTester. You may change your database names, but
 * ensure that you grant privileges and adjust the urls accordingly.
 * <p>
 * Test sets contain 'if(false)' blocks for inserting the data set. If you have
 * restored the data from the testSearchData dump file, leave these alone. If
 * you have not done this, change all conditions to true, run the class ONCE,
...
public class DatabaseSearchTests {
  ...
  public void testSet1() throws SQLException, MissingDataException {
    if (false) {
      List<String> nameList = new ArrayList<>(Arrays.asList("1A", "1B", "1C",
          "1D", "1E", "1F", "1G", "1H", "1I", "1J", "1K", "1L"));
      double[] lat1List = {
          40.005, 40.01, 40.5, 40.5, 40.494, 40.491, 41.003, 39.996, 42.999,
          43.003, 41, 41.5
      };
      double[] lon1List = {
          40.005, 39.989, 40.507, 40.5, 40.501, 40.491, 41.004, 39.998, 40.001,
          40.002, 41.007, 41.002
      };
      double[] lat2List = {
          40.5, 40.5, 41, 41, 41, 41, 42, 43, 40, 42, 41.5, 42
      };
      double[] lon2List = {
          40.5, 40.5, 41, 41, 41, 41, 41, 40, 40, 41, 41, 41
      };
      long[] departureList = {
          300, 300, 5000, 5000, 5000, 5000, 7500, 300, 8800, 8800, 7500, 10000
      };
      long[] etaList = {
          4000, 4000, 7000, 7000, 7000, 7000, 9500, 8000, 20000, 12000, 9500,
          12000
      };
    ...
    List<Trip> s1 = searchTester.getConnectedTripsWithinTimeRadius(40, 40, 0.85,
        300, 1500);
    assertTrue(hasAllTrips(new ArrayList<>(Arrays.asList("1A", "1H")), s1));

    List<Trip> s2 = searchTester.getConnectedTripsWithinTimeRadius(40, 40, 1.6,
        300, 1500);
    assertTrue(
  ...
  public void testSet2() throws SQLException, MissingDataException {
    if (false) {
      searchTester.createTrip(
          Trip.TripBuilder.newTripBuilder().addIdentification(-1, "2A")
              ...
              .addTimes(50, 300).addDetails(2, 2.00, "", "", "").build(),
          "1");
      searchTester.createTrip(Trip.TripBuilder.newTripBuilder()
          .addIdentification(-1, "2B")
          .addLocations(-0.001, -0.004, 0.352, 0.347).addAddressNames("", "")
      ...
      searchTester.createTrip(
          Trip.TripBuilder.newTripBuilder().addIdentification(-1, "2D")
              ...
              .addTimes(50, 1000).addDetails(2, 2.00, "", "", "").build(),
          "1");
      searchTester.createTrip(
    ...
    // these conditions should give paths that traverse the entire set
    // specific paths to follow soon
    List<Trip> s1 = searchTester.getConnectedTripsWithinTimeRadius(0, 0, 1.3,
        50, 2000);
    assertTrue(
  ...
  public void testSet3() throws SQLException, MissingDataException {
    if (false) {
      List<String> nameList = new ArrayList<>(Arrays.asList("3A", "3B", "3C",
          "3D", "3E", "3F", "3G", "3H", "3I", "3J", "3K", "3L"));
      double[] lat1List = {
          -16, -16, -16, -16.5, -16.5, -16.5, -16.7, -16.7, -16.7, -16.7, -16,
          -16
      };
      double[] lon1List = {
          -16, -16, -16, -16.5, -16.5, -16.5, -16.8, -16.8, -16.8, -16.8, -16,
          -16
      };
      double[] lat2List = {
          -16.5, -16.5, -16.5, -16.7, -16.7, -16.7, -17, -17, -17, -17, -16.7,
          -16.7
      };
      double[] lon2List = {
          -16.5, -16.5, -16.5, -16.8, -16.8, -16.8, -17, -17, -17, -17, -16.8,
          -16.8
      };
      long[] departureList = {
          10, 20, 30, 125, 140, 150, 205, 235, 240, 225, 15, 25
      };
      long[] etaList = {
          130, 120, 135, 200, 230, 225, 500, 505, 590, 800, 230, 200
      };
      for (int i = 0; i < nameList.size(); i++) {
        searchTester.createTrip(Trip.TripBuilder.newTripBuilder()
            .addIdentification(-1, nameList.get(i))
  ...
  public void testSet4() throws SQLException, MissingDataException {
    if (false) {
      List<String> nameList = new ArrayList<>(
          Arrays.asList("4A", "4B", "4C", "4D", "4E", "4F", "4G", "4H"));
      double[] lat1List = {
          75, 81, 81, 81, 82, 81, 81, 81
      };
      double[] lon1List = {
          100, 100, 100, 105, 106, 107, 105, 105
      };
      double[] lat2List = {
          80, 82, 80, 81.9, 83, 83, 83, 83
      };
      double[] lon2List = {
          100, 105, 105, 105, 110, 110, 110, 110
      };
      long[] departureList = {
          0, 80000, 80000, 160000, 1740000, 160000, 0, 1000000
      };
      long[] etaList = {
          1, 162000, 80001, 160001, 200000, 200000, 200000, 1000001
      };
      for (int i = 0; i < nameList.size(); i++) {
        searchTester.createTrip(Trip.TripBuilder.newTripBuilder()
            .addIdentification(-1, nameList.get(i))
