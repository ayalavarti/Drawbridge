...
public class CarpoolsTest {
  ...
  public void testGetTripInvalid() throws SQLException, MissingDataException {
  ...
  @Test
  public void testJoinAndLeaveTrip() throws SQLException, MissingDataException {
  ...
  @Test
  public void testApproveRequest() throws SQLException, MissingDataException {
    assertTrue(carpools.joinTrip(t1, "3"));
  ...
  @Test
  public void testRejectRequest() throws SQLException, MissingDataException {
    assertTrue(carpools.joinTrip(t1, "3"));
  ...
  public void testGetUsers() throws SQLException, MissingDataException {
    ...
    List<List<User>> expected = new ArrayList<List<User>>();
    expected.add(new ArrayList<User>(Arrays.asList(DUMMY_U1)));
    expected.add(new ArrayList<User>(Arrays.asList(DUMMY_U2)));
    expected.add(new ArrayList<User>(Arrays.asList(DUMMY_U3)));
  ...
  public void testGetTrips() throws SQLException, MissingDataException {
    ...
    expected.add(new ArrayList<Trip>(Arrays.asList(DUMMY_T1)));
    expected.add(new ArrayList<Trip>(Arrays.asList(DUMMY_T2)));
    expected.add(new ArrayList<Trip>(Arrays.asList(DUMMY_T3)));
  ...
  public void testSearch() throws SQLException, MissingDataException {
    List<List<Trip>> expected = new ArrayList<List<Trip>>();
    expected.add(new ArrayList<Trip>(Arrays.asList(DUMMY_T1)));
    List<List<Trip>> actual = carpools.searchWithoutId(3, 3, 5, 5, 1000, 15,
        30);
    for (int i = 0; i < expected.size(); i++) {
      for (int j = 0; j < expected.get(i).size(); j++) {
        assertEquals(expected.get(i).get(j).getName(),
